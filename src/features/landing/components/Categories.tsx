      ...
      <Container>
        ...
        <div className="hz-category-grid grid grid-cols-2 gap-x-3 gap-y-2 sm:grid-cols-3 lg:grid-cols-5 lg:gap-x-5 lg:gap-y-1">
          {PROPERTY_CATEGORIES.map((item) => {
            ...
            const isNature = NATURE_ICONS.has(item.icon);

            return (
              <Link
                key={item.id}
                href="/cabins"
                className="hz-category-card group flex min-h-[82px] items-center gap-3 rounded-2xl px-2 py-3 transition-all duration-300 hover:bg-surface-raised sm:px-3"
              >
                <span
                  className={`hz-category-icon flex size-12 shrink-0 items-center justify-center rounded-xl border border-border bg-surface text-text transition-all duration-300 group-hover:-translate-y-0.5 ${
                    isNature
                      ? "group-hover:border-secondary-400/40 group-hover:bg-secondary-400/10 group-hover:text-secondary-500"
                      : "group-hover:border-primary-400/40 group-hover:bg-primary-400/10 group-hover:text-primary-500"
                  }`}
                >
                ...
                <span className="min-w-0 text-right">
                  <span
                    className={`block truncate text-sm font-bold text-text transition-colors ${
                      isNature ? "group-hover:text-secondary-500" : "group-hover:text-primary-500"
                    }`}
                  >
                    {item.title}
                  </span>
